...
export const responderQuestao = async (req: AuthenticatedRequest, res: Response) => {
  ...
  pool.query('SELECT resposta_correta, explicacao FROM questoes WHERE id = ?', [questao_id], (err, results: any) => {
    if (err) {
    ...
    pool.query(insertSql, [usuario_id, questao_id, letraResposta, correta], (insertErr) => {
      if (insertErr) {
...
export const getCategorias = async (req: AuthenticatedRequest, res: Response) => {
  pool.query('SELECT DISTINCT categoria FROM questoes WHERE categoria IS NOT NULL', (err, results: any) => {
    if (err) {
